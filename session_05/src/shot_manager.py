...
# ============================================================
# SHOT MANAGER — Sessione 04.1 + 04.2 + 05
...
#         fallback: summary
# 05   → crea shots.json con file_path, frame_start, frame_end
# ============================================================
...
# ============================================================
# FUNZIONI DI SESSIONE 04.2 (invariate)
# ============================================================
...
# ============================================================
# FUNZIONE NUOVA — SESSIONE 05
# ============================================================

def create_manifest_command(folder, output_path):
    """Scansiona una cartella di EXR e scrive shots.json con i tre nuovi campi."""
    files = list_files(folder)

    if not files:
        print(f"Nessun file EXR trovato in: {folder}")
        sys.exit(1)

    # Raggruppa i file per (show, seq, shot, dept, version)
    groups = {}
    for f in files:
        show, seq, shot, dept, version, frame = parse_filename(f)
        key = f"{show}_{seq}_{shot}_{dept}_{version}"
        ...
        groups[key]["frames"].append(int(frame))

    # Costruisce un record per ogni gruppo
    records = []
    for key, g in sorted(groups.items()):
        frame_start = min(g["frames"])
        frame_end   = max(g["frames"])

        # Cerca un manifest esistente per status, user e notes
        manifest_key = f"{g['show']}_{g['sequence']}_{g['shot']}"
        ...
        record = {
            ...
            "status":      manifest["status"]      if manifest else "waiting",
            "notes":       manifest["notes"]       if manifest else "",
            "last_update": manifest["last_update"] if manifest else "",
            "file_path":   os.path.join(folder, f"{g['show']}_{g['sequence']}_{g['shot']}_{g['department']}_{g['version']}.%04d.exr"),
    ...
    print(f"{len(records)} shot scritti in: {output_path}")
...
if len(sys.argv) < 2:
    ...
    print("Comandi disponibili: list, count, shot, summary, create-manifest")
    sys.exit(1)
...
elif command == "create-manifest":
    folder      = sys.argv[2]
    output_path = sys.argv[3]
    create_manifest_command(folder, output_path)
